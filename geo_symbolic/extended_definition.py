...
class ExtendedDefinition(BasicDefinition):
    ...
    def parseAngle(self, angle):
        if len(angle) == 3:
            ...
            angle = angle[0]
            polygons = self.find_polygons_with_point(angle)
            if len(polygons) > 0:
                p = polygons[0]
                index = 0
                n = len(p)
                for i in range(n):
                    if p[i] == angle:
                        index = i
                        break
                return [p[(index-1)%n], p[index], p[(index+1)%n]]
    ...
    def definePolygon(self, points):
        ...
        if l == 3:
            ...
            facts(self.Hexagon, sort_points(points))
    ...
    def newRadiusSymbol(self, expr):
        sym = Symbol("RadiusOf({})".format(expr))
        return sym
    ...
    def find_polygons_with_point(self, point):
        x = [var() for i in range(5)]
        res = run(0, tuple(x), self.Pentagon(*x), membero(point, tuple(x)))
        if len(res) > 0:
            return res
        x = [var() for i in range(4)]
        res = run(0, tuple(x), self.Quadrilateral(*x), membero(point, tuple(x)))
        if len(res) > 0:
            return res
        x = [var() for i in range(3)]
        res = run(0, tuple(x), self.Triangle(*x), membero(point, tuple(x)))
        if len(res) > 0:
            return res
        return []
