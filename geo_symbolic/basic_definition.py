...
from utils import isNumber, hasNumber
...
class BasicDefinition(object):
    def __init__(self, debug):
        ...
        self.Hexagon = Relation()

        self.Parallelogram = Relation()
        self.Rhombus = Relation()
        self.Rectangle = Relation()
        self.Square = Relation()
        self.Trapezoid = Relation()
    ...
    def define_parallel_by_point(self, point_A, point_B, point_C, point_D):
        res = run(1, (), self.Parallel(point_A, point_B, point_C, point_D))
        if len(res) > 0:
            return False
        self.define_line(point_A, point_B)
        self.define_line(point_C, point_D)
        facts(self.Parallel, (point_A, point_B, point_C, point_D))
        return True
    ...
    def define_perpendicule(self, line1, line2):
        # Define two lines Perpendicule
        facts(self.Perpendicule, (line1[0], line1[1], line2[0], line2[1]))
    
    def define_parallelogram(self, point_A, point_B, point_C, point_D):
        res = run(1, (), self.Parallelogram(point_A, point_B, point_C, point_D))
        if len(res) > 0:
            return False
        facts(self.Parallelogram, (point_A, point_B, point_C, point_D))
        return True
    
    def define_rhombus(self, point_A, point_B, point_C, point_D):
        res = run(1, (), self.Rhombus(point_A, point_B, point_C, point_D))
        if len(res) > 0:
            return False
        facts(self.Rhombus, (point_A, point_B, point_C, point_D))
        return True
    
    def define_rectangle(self, point_A, point_B, point_C, point_D):
        res = run(1, (), self.Rectangle(point_A, point_B, point_C, point_D))
        if len(res) > 0:
            return False
        facts(self.Rectangle, (point_A, point_B, point_C, point_D))
        return True
    
    def define_square(self, point_A, point_B, point_C, point_D):
        res = run(1, (), self.Square(point_A, point_B, point_C, point_D))
        if len(res) > 0:
            return False
        facts(self.Square, (point_A, point_B, point_C, point_D))
        return True
    
    def define_trapezoid(self, point_A, point_B, point_C, point_D):
        res = run(1, (), self.Trapezoid(point_A, point_B, point_C, point_D))
        if len(res) > 0:
            return False
        facts(self.Trapezoid, (point_A, point_B, point_C, point_D))
        return True
    ...
    def check_perpendicular(self, line1, line2):
        res = len(run(1, (), self.Perpendicule(line1[0], line1[1], line2[0], line2[1]))) > 0 or \
              len(run(1, (), self.Perpendicule(line1[0], line1[1], line2[1], line2[0]))) > 0 or \
              len(run(1, (), self.Perpendicule(line1[1], line1[0], line2[0], line2[1]))) > 0 or \
              len(run(1, (), self.Perpendicule(line1[1], line1[0], line2[1], line2[0]))) > 0 or \
              len(run(1, (), self.Perpendicule(line2[0], line2[1], line1[0], line1[1]))) > 0 or \
              len(run(1, (), self.Perpendicule(line2[0], line2[1], line1[1], line1[0]))) > 0 or \
              len(run(1, (), self.Perpendicule(line2[1], line2[0], line1[0], line1[1]))) > 0 or \
              len(run(1, (), self.Perpendicule(line2[1], line2[0], line1[1], line1[0]))) > 0
        if res: return True
    ...
    def check_parallelogram(self, A, B, C, D):
        res = run(1, (), self.Parallelogram(A, B, C, D))
        return len(res) > 0
    
    def check_rhombus(self, A, B, C, D):
        res = run(1, (), self.Rhombus(A, B, C, D))
        return len(res) > 0
    
    def check_rectangle(self, A, B, C, D):
        res = run(1, (), self.Rectangle(A, B, C, D))
        return len(res) > 0
    
    def check_square(self, A, B, C, D):
        res = run(1, (), self.Square(A, B, C, D))
        return len(res) > 0
    
    def check_trapezoid(self, A, B, C, D):
        res = run(1, (), self.Trapezoid(A, B, C, D))
        return len(res) > 0
    ...
    def find_all_parallelograms(self):
        x = var()
        y = var()
        z = var()
        w = var()

        res = run(0, (x, y, z, w), self.Parallelogram(x, y, z, w))
        return list(res)
    
    def find_all_rhombuss(self):
        x = var()
        y = var()
        z = var()
        w = var()

        res = run(0, (x, y, z, w), self.Rhombus(x, y, z, w))
        return list(res)
    ...
    def find_all_rectangles(self):
        ...
        res = run(0, (x, y, z, w), self.Rectangle(x, y, z, w))
        return list(res)
    ...
    def find_all_squares(self):
        ...
        res = run(0, (x, y, z, w), self.Square(x, y, z, w))
        return list(res)
    
    def find_all_trapezoids(self):
        x = var()
        y = var()
        z = var()
        w = var()

        res = run(0, (x, y, z, w), self.Trapezoid(x, y, z, w))
        return list(res)
    ...
    def find_all_hexagons(self):
        ...
        res = run(0, (x, y, z, s, t, r), self.Hexagon(x, y, z, s, t, r))
        return list(res)
    ...
    def find_all_perpendicule(self):
        x = var()
        y = var()
        z = var()
        w = var()

        res = run(0, (x, y, z, w), self.Perpendicule(x, y, z, w))
        return [((t[0], t[1]), (t[2], t[3])) for t in list(res)]
